...
public class ClimbSubsystem extends SubsystemBase {
    ...
    public void periodic() {
        ...
        if (
                (brakeTarget.equals(BrakeState.Engaged) && powerDist.getCurrent(Constants.Climb.BRAKE_POWER) >= Constants.Climb.BRAKE_OVERDRAW_THRESHOLD)
                        || (brakeTarget.equals(BrakeState.Released) && brakeTransitionTimer > (int) (Constants.Climb.BRAKE_TIMEOUT * 50))) {
            brake.stopMotor();
    ...
    /**
     * This function never blocks, instead setting the brake state to transitioning until the transition is finished.
     */
